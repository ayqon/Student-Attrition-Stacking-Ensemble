"""
KNN Classification - 4-Class Student Outcome Prediction
...
output_dir = os.path.join(base_path, "output", "KNN_outputs")
...
# ==================== Baseline KNN Model ====================
print("\n🚀 Training Baseline KNN (k=5)...")

baseline_model = KNeighborsClassifier(n_neighbors=5, n_jobs=1)
...
print(f"   5-fold CV: {total_combinations * 5} total fits")
...
print(f"   Best K from analysis: {best_k_idx + 1} with accuracy {k_scores[best_k_idx]:.4f}")
...
ax4 = axes[1, 1]
ax4.bar(class_names, f1_per_class, color=['#3498DB', '#E74C3C', '#2ECC71', '#F39C12'], alpha=0.8, edgecolor='black')
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'knn_results.png'), dpi=300, bbox_inches='tight')
plt.close()
print("   ✅ knn_results.png")

# 2. K-Value Analysis Plot
fig, ax = plt.subplots(figsize=(12, 6))
ax.plot(list(k_range), k_scores, marker='o', linewidth=2, markersize=6, color='#3498DB')
ax.axvline(x=grid_search.best_params_['n_neighbors'], color='red', linestyle='--', 
           label=f'Best K={grid_search.best_params_["n_neighbors"]}')
ax.set_xlabel('Number of Neighbors (K)', fontsize=12)
ax.set_ylabel('Test Accuracy', fontsize=12)
ax.set_title('KNN: K-Value vs Accuracy', fontweight='bold', fontsize=14)
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'k_value_analysis.png'), dpi=300, bbox_inches='tight')
plt.close()
print("   ✅ k_value_analysis.png")

# 3. Overall Accuracy Summary Table
fig, ax = plt.subplots(figsize=(12, 8))
ax.axis('off')
title_text = 'KNN - OVERALL ACCURACY SUMMARY\n4-Class Classification'
...
table_data = [
    ...
    ['Best K', f'{grid_search.best_params_["n_neighbors"]}'],
    ['Best Weights', f'{grid_search.best_params_["weights"]}'],
    ['Best Metric', f'{grid_search.best_params_["metric"]}'],
    ['Test Samples', f'{len(y_test):,}'],
]

table = ax.table(cellText=table_data, cellLoc='center', loc='center',
                colWidths=[0.4, 0.4], bbox=[0.1, 0.25, 0.8, 0.6])
...
for i in range(5):
    class_table[(0, i)].set_facecolor('#3498DB')
...
k_analysis_df = pd.DataFrame({
    'K': list(k_range),
    'Accuracy': k_scores
})
k_analysis_df.to_csv(os.path.join(output_dir, 'k_value_analysis.csv'), index=False)
print("   ✅ k_value_analysis.csv")

model_info_df = pd.DataFrame({
    'Property': ['Model Type', 'Best K', 'Best Weights', 'Best Metric', 'Accuracy', 'F1-Score', 
                 'Train Samples', 'Test Samples', 'Num Classes', 'Classes', 'Training Date'],
    'Value': ['KNN', grid_search.best_params_['n_neighbors'], grid_search.best_params_['weights'],
              grid_search.best_params_['metric'], f'{best_acc:.4f}', f'{best_f1:.4f}', 
              len(y_train), len(y_test), len(class_names), ', '.join(class_names), 

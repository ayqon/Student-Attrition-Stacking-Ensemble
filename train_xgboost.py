"""
XGBoost Classification - 4-Class Student Outcome Prediction
...
output_dir = os.path.join(base_path, "output", "XG Boost_outputs")
...
# Encode string labels to numeric (XGBoost requires 0-indexed labels)
...
scaler_path = os.path.join(output_dir, 'xgboost_scaler.pkl')
...
ax1.set_ylim([0, 1])
ax1.set_title('XGBoost - Model Comparison', fontweight='bold')
for bar, acc in zip(bars, acc_list):
    ax1.text(bar.get_x() + bar.get_width()/2, acc + 0.02, f'{acc:.4f}', 
             ha='center', va='bottom', fontweight='bold')

ax2 = axes[0, 1]
sns.heatmap(cm, annot=True, fmt='d', cmap='Oranges', ax=ax2, cbar=False,
...
width = 0.35
bars1 = ax3.bar(x - width/2, precision_per_class, width, label='Precision', color='#F39C12', alpha=0.8)
bars2 = ax3.bar(x + width/2, recall_per_class, width, label='Recall', color='#9B59B6', alpha=0.8)
...
ax4.barh(range(len(top_features)), top_features['Importance'], color='#E67E22', alpha=0.8, edgecolor='black')
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'xgboost_results.png'), dpi=300, bbox_inches='tight')
plt.close()
...
ax.axis('off')
title_text = 'XGBOOST - OVERALL ACCURACY SUMMARY\n4-Class Classification'
ax.text(0.5, 0.95, title_text, ha='center', va='top', fontsize=16, fontweight='bold',
        transform=ax.transAxes, bbox=dict(boxstyle='round', facecolor='#F39C12', alpha=0.6))
...
table_data = [
    ...
    ['Best learning_rate', f'{grid_search.best_params_["learning_rate"]}'],
...
for i in range(2):
    table[(0, i)].set_facecolor('#E67E22')
...
ax.text(0.5, 0.95, title_text, ha='center', va='top', fontsize=16, fontweight='bold',
        transform=ax.transAxes, bbox=dict(boxstyle='round', facecolor='#9B59B6', alpha=0.6))
...
for i in range(5):
    class_table[(0, i)].set_facecolor('#9B59B6')
    class_table[(0, i)].set_text_props(weight='bold', color='white')

colors_rows = ['#F5EEF8', '#FDEBD0', '#FCF3CF', '#D5D8DC']
...
model_info_df = pd.DataFrame({
    'Property': ['Model Type', 'Best n_estimators', 'Best max_depth', 'Best learning_rate',
                 'Best min_child_weight', 'Best subsample', 'Best colsample_bytree',
                 'Accuracy', 'F1-Score', 'Train Samples', 'Test Samples', 
                 'Num Classes', 'Classes', 'Training Date'],
    'Value': ['XGBoost', grid_search.best_params_['n_estimators'], 
              grid_search.best_params_['max_depth'], grid_search.best_params_['learning_rate'],
              grid_search.best_params_['min_child_weight'], grid_search.best_params_['subsample'],

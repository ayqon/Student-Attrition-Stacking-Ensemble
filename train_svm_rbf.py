"""
SVM Classification with RBF Kernel - 4-Class Student Outcome Prediction
...
output_dir = os.path.join(base_path, "output", "SVM_outputs")
...
baseline_model = SVC(kernel='rbf', C=1.0, gamma='scale', random_state=RANDOM_STATE)
...
ax1.set_ylim([0, 1])
ax1.set_title('SVM RBF - Model Comparison', fontweight='bold')
...
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'svm_rbf_results.png'), dpi=300, bbox_inches='tight')
plt.close()
print("   ✅ svm_rbf_results.png")

# 2. Overall Accuracy Summary Table
fig, ax = plt.subplots(figsize=(12, 8))
ax.axis('off')
title_text = 'SVM RBF - OVERALL ACCURACY SUMMARY\n4-Class Classification'
...
table_data = [
    ...
    ['Best C', f'{grid_search.best_params_["C"]}'],
    ['Best gamma', f'{grid_search.best_params_["gamma"]}'],
...
model_info_df = pd.DataFrame({
    'Property': ['Model Type', 'Kernel', 'Best C', 'Best gamma', 'Accuracy', 'F1-Score', 
                 'Train Samples', 'Test Samples', 'Num Classes', 'Classes', 'Training Date'],
    'Value': ['SVM', 'RBF', grid_search.best_params_['C'], grid_search.best_params_['gamma'],
